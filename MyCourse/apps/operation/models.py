...
class CoursComments(models.Model):
    '''
         課程評論
    '''
...
class UserMessage(models.Model):
    "用戶信息"
    # 这里用的是int类型来存储默认值是0就是当这个值是0时就是默认给所有用户发送的值，当不是的话就是那个用户的id就是单个给用户发送的值
    user = models.IntegerField(default=0,verbose_name=u"接收用户")
    message = models.CharField(max_length=500,verbose_name=u"添加消息")
    # 前面用has來命名的話就是bool類型是否讀過,就是发送的消息是否已读,默认是没有读的因为这个消息是刚发送过来的
